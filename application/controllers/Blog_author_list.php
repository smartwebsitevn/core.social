...
class Blog_author_list extends MY_Controller
{
    public function __construct()
    {
        parent::__construct();
        // Tai cac file thanh phan
        $this->lang->load('site/blog_author');
    ...
    public function category($id = null)
    {
        ...
        if (!is_numeric($id) && is_slug($id)) {
            // neu la seo url
            $category = model("blog_author_cat")->get_info_rule(array('seo_url' => $id, 'status' => 1));
        } else {
            $category = model("blog_author_cat")->get_info_rule(array('id' => $id, 'status' => 1));
        }
        if (!$category)
            show_404();
        $category = mod("blog_author_cat")->add_info($category);
        $this->data['category'] = $category;

        // Filter set
        $filter = array();
        $filter['cat_id'] = mod('blog_author_cat')->get_child_ids($category->id);
    ...
    public function owner()
    {
        ...
        //  $this->data['list'] = model('blog_author')->filter_get_list($filter, $input);
        $this->_create_list($input, $filter);

        // khoa hoc mien phi
        $blog_authors_free = model('blog_author')->filter_get_list([
            'price_option' => 0
        ], ['order' => array('created', 'DESC')]);
        //pr_db();
        if ($blog_authors_free)
            foreach ($blog_authors_free as $row) {
                $row = mod('blog_author')->add_info($row);
            }

        $this->data['blog_authors_free'] = $blog_authors_free;
        // Authors
        $blog_author_authors = array_gets($this->data['list'], 'author_id');
        $author_ids = array_merge($blog_author_authors);

        $this->data['authors'] = null;
        if ($author_ids)
            $this->data['authors'] = model('blog_author_author')->filter_get_list(['id' => $author_ids, 'show' => 1]);


        page_info('breadcrumbs', array('#', lang('my_blog_authors'), lang('my_blog_authors')));
        page_info('title', lang('my_blog_authors'));
        $this->_display();
    }
    ...
    {
        $filter_input = array();
        $filter_fields = array_merge($filter_fields, model('blog_author')->fields_filter);
        $mod_filter = mod('blog_author')->create_filter($filter_fields, $filter_input);
        ...
        if (!isset($input['limit'])) {
            $total = model('blog_author')->filter_get_total($filter, $input);
            // pr_db();
        ...
        $list = model('blog_author')->filter_get_list($filter, $input);
        //pr_db($list);
        foreach ($list as $row) {
            $row = mod('blog_author')->add_info($row);
        ...
        /* $this->data['types'] = mod("blog_author")->config('movie_types');
        ...
        mod('blog_author')->sess_data_set('list_filter', $filter);// phuc vu loc du lieu
        mod('blog_author')->sess_data_set('list_filter_input', $filter_input);// phuc vu hien thi
        mod('blog_author')->sess_data_set('list_sort_orders', $sort_orders);// phuc vu hien thi
        mod('blog_author')->sess_data_set('list_sort_order', $order);// phuc vu hien thi
        mod('blog_author')->sess_data_set('list_total_rows', $total);// phuc vu hien thi
    ...
    private function _create_list_ajax()
    {
        if ($this->input->is_ajax_request()) {
            ...
            if (isset($this->data['category'])) {
                ...
                if ($cat_id)
                    $category = model("blog_author_cat")->get_info($cat_id);
            }
            ...
            echo json_encode(
                array(
                    'status' => true,
                    'content' => widget('blog_author')->display_list(
                        $this->data['list'], $temp,
                        array(
                            'return_data' => 1,
