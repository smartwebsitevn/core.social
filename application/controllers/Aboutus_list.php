...
class Aboutus_list extends MY_Controller
{
    public function __construct()
    {
        parent::__construct();
        // Tai cac file thanh phan
        $this->lang->load('site/aboutus');
    ...
    public function category($id = null)
    {
        ...
        if (!is_numeric($id) && is_slug($id)) {
            // neu la seo url
            $category = model("aboutus_cat")->filter_get_info(array('seo_url' => $id, 'show' => 1));
        } else {
            $category = model("aboutus_cat")->filter_get_info(array('id' => $id, 'show' => 1));
        }
        if (!$category)
            show_404();
        $category = mod("aboutus_cat")->add_info($category);
        $this->data['category'] = $category;

        // Filter set
        $filter = array();
        $filter['cat_id'] = mod('aboutus_cat')->get_child_ids($category->id);
    ...
    public function owner()
    {
        ...
        //  $this->data['list'] = model('aboutus')->filter_get_list($filter, $input);
        $this->_create_list($input, $filter);

        // khoa hoc mien phi
        $aboutuss_free = model('aboutus')->filter_get_list([
            'price_option' => 0
        ], ['order' => array('created', 'DESC')]);
        //pr_db();
        if ($aboutuss_free)
            foreach ($aboutuss_free as $row) {
                $row = mod('aboutus')->add_info($row);
            }

        $this->data['aboutuss_free'] = $aboutuss_free;
        // Authors
        $aboutus_authors = array_gets($this->data['list'], 'author_id');
        $author_ids = array_merge($aboutus_authors);

        $this->data['authors'] = null;
        if ($author_ids)
            $this->data['authors'] = model('aboutus_author')->filter_get_list(['id' => $author_ids, 'show' => 1]);


        page_info('breadcrumbs', array('#', lang('my_aboutuss'), lang('my_aboutuss')));
        page_info('title', lang('my_aboutuss'));
        $this->_display();
    }
    ...
    {
        $filter_input = array();
        $filter_fields = array_merge($filter_fields, model('aboutus')->fields_filter);
        $mod_filter = mod('aboutus')->create_filter($filter_fields, $filter_input);
        ...
        if (!isset($input['limit'])) {
            $total = model('aboutus')->filter_get_total($filter, $input);
            // pr_db();
       ...
       /* if (!isset($input['order'])) {
        ...
        $list = model('aboutus')->filter_get_list($filter, $input);
        //pr_db($list);
        foreach ($list as $row) {
            $row = mod('aboutus')->add_info($row);
        ...
        /* $this->data['types'] = mod("aboutus")->config('movie_types');
        ...
        mod('aboutus')->sess_data_set('list_filter', $filter);// phuc vu loc du lieu
        mod('aboutus')->sess_data_set('list_filter_input', $filter_input);// phuc vu hien thi
        mod('aboutus')->sess_data_set('list_sort_orders', $sort_orders);// phuc vu hien thi
        mod('aboutus')->sess_data_set('list_sort_order', $order);// phuc vu hien thi
        mod('aboutus')->sess_data_set('list_total_rows', $total);// phuc vu hien thi
    ...
    private function _create_list_ajax()
    {
        if ($this->input->is_ajax_request()) {
            ...
            if (isset($this->data['category'])) {
                ...
                if ($cat_id)
                    $category = model("aboutus_cat")->get_info($cat_id);
            }
            ...
            echo json_encode(
                array(
                    'status' => true,
                    'content' => widget('aboutus')->display_list(
                        $this->data['list'], $temp,
                        array(
                            'return_data' => 1,
