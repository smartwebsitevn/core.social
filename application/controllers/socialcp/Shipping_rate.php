...
class Shipping_rate extends MY_Controller {
	public $params = array(
	...
	public $filter = array( 'name', 'geo_zone_id', 'show' );
		...
		if ($this->input->post('_submit'))
		{
			...
			if ($this->form_validation->run())
			{
				...
				foreach ($this->params as $key => $value) {
					if( $key == 'cost' )
					{
						$cost = (String)$this->input->post( $key );
				...
				$result['location'] = admin_url('shipping_rate');
	...
	function index()
	{
		...
		foreach ($list as $row)
		{
			...
			$row->_url_translate = admin_url("translate/table/shipping_rate/{$row->id}");
