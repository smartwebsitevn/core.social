...
class Download extends MY_Controller {
	...
	public $columes = array('name', 'summary', 'content',
			'order','feature','status','url_files','lang_id',
 ...
 */
	...
	function _set_rules($params = array())
	{
		...
		$rules['summary'] 		= array('summary', 'trim|xss_clean');
		$rules['url_files'] 		= array('url_files', 'trim|xss_clean|callback__check_url_files');
	...
	// chuya lam xong
	function _check_url_files($value){
		if(!$value)
			return true;
	...
	private function _get_input($info = null){
		...
		$data['url'] = $this->getUrl(convert_vi_to_en($data['url']), $info ? $info->id : 0);
	...
	function add()
	{
		...
		if ($this->input->post('_submit'))
		{
			...
			if ($this->form_validation->run())
			{
				...
				// lay tong dung luong
				$files = model('file')->totalSizeFile(__CLASS__, $fake_id, 'files');
				$data['total_sizes'] = $files['sizes'];
				$data['total_files'] = $files['files'];
				$id = 0;
	...
	function _edit($info)
	{
		...
		if ($this->input->post('_submit'))
		{
			...
			if ($this->form_validation->run())
			{
				...
				$files = model('file')->totalSizeFile(__CLASS__, $info->id, 'files');
				$data['total_sizes'] = $files['sizes'];
				$data['total_files'] = $files['files'];
 ...
 */
	...
	function index()
	{
		...
		// Tao filter
		$filter_input 	= array();
		$filter_fields 	= array('id', 'name','lang_id', 'feature', 'category','status');
