...
class Blog_author extends MY_Controller
{
    ...
    function index()
    {
        redirect(site_url('blog_author_list'));
    }
    ...
    function _common_process()
    {
        ...
        if (!is_numeric($id) && is_slug($id)) {
            // neu la seo url
            $info = $this->_model()->get_info_rule(array('seo_url' => $id));
        } else {
        ...
        //== Su ly danh muc
        $category = model('blog_author_cat')->get_info($info->cat_id);
    ...
    protected function _breadcrumbs($category)
    {
        ...
        while ($parent->parent_id) {
            $parent = model('blog_author_cat')->get_info($parent->parent_id);
