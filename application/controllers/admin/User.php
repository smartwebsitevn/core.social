 ...
 */
	...
	function _get_image($id)
	{
		...
		$image = $this->file_model->get_info_of_mod('user', $id, 'avatar', 'id, file_name');
	...
	function add()
	{
		...
		if ($this->input->post('_submit'))
		{
			...
			if ($this->form_validation->run())
			{
				...
				// Xu ly password
				$password = $this->input->post('password');
				$password = security_encode($password, strtolower($email));
				...
				$user_id =0;
				$this->user_model->create($data,$user_id);


				// Cap nhat lai table_id cua image trong table file
				$this->file_model->update_table_id_of_mod('user', $fake_id, $user_id);
