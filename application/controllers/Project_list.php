...
class Project_list extends MY_Controller
{
    public function __construct()
    {
        ...
        $this->lang->load('site/project');

    }

    /**
     * All project page display
     *
     *
     */
    ...
    public function category($id = null)
    {
        ...
        if (!is_numeric($id) && is_slug($id)) {
            // neu la seo url
            $category = model("project_cat")->filter_get_info(array('seo_url' => $id, 'show' => 1));
        } else {
            $category = model("project_cat")->filter_get_info(array('id' => $id, 'show' => 1));
        }
        if (!$category)
            show_404();
        $category = mod("project_cat")->add_info($category);
        $this->data['category'] = $category;
        // Filter set
        $filter = array();
        $filter['cat_id'] = mod('project_cat')->get_child_ids($category->id);
    ...
    public function owner()
    {
        ...
        //  $this->data['list'] = model('project')->filter_get_list($filter, $input);
        $this->_create_list($input, $filter);

        // khoa hoc mien phi
        $projects_free = model('project')->filter_get_list([
            'price_option' => 0
        ], ['order' => array('created', 'DESC')]);
        //pr_db();
        if ($projects_free)
            foreach ($projects_free as $row) {
                $row = mod('project')->add_info($row);
            }

        $this->data['projects_free'] = $projects_free;
        // Authors
        $project_authors = array_gets($this->data['list'], 'author_id');
        $author_ids = array_merge($project_authors);

        $this->data['authors'] = null;
        if ($author_ids)
            $this->data['authors'] = model('project_author')->filter_get_list(['id' => $author_ids, 'show' => 1]);


        page_info('breadcrumbs', array('#', lang('my_projects'), lang('my_projects')));
        page_info('title', lang('my_projects'));
        $this->_display();
    }
    ...
    {
        $filter_input = array();
        $filter_fields = array_merge($filter_fields, model('project')->fields_filter);
        $mod_filter = mod('project')->create_filter($filter_fields, $filter_input);
        ...
        if (!isset($input['limit'])) {
            $total = model('project')->filter_get_total($filter, $input);
            // pr_db();
        ...
        $list = model('project')->filter_get_list($filter, $input);
        //pr_db($list);
        foreach ($list as $row) {
            $row = mod('project')->add_info($row);
            $row =mod('project')->add_info_images($row);
        }
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if (isset($this->data['category'])) {
                ...
                if ($cat_id)
                    $category = model("project_cat")->get_info($cat_id);
            }
            ...
            echo json_encode(array('status' => true,
                'content' => widget('project')->display_list($list, $temp, array('return_data' => 1, 'pages_config' => $this->data['pages_config'])),
        ...
        $categories=model('project_cat')->filter_get_list(['show'=>1]);
        foreach($categories as $row){
            $row =mod('project_cat')->add_info_url($row);
        }
        $this->data['categories'] =$categories;


        // lay cac loai danh muc
        /* $this->data['types'] = mod("project")->config('movie_types');
