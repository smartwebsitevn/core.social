...
class Sms extends MY_Controller {
	...
	public function _t()
	{
		...
		$data['smsid']		= now();
		$data['content'] 	= $sms[0];
		$data['sender'] 	= '0123456789';
		$data['serviceid'] 	= $sms[1];
		
		$url = site_url('sms').'?'.http_build_query($data);
		pr($url);
	...
	{
		// Kiem tra ip
		$service_ip = lib('sms')->get_service_ip();
		...
		// Luu sms
		model('sms_receive')->create(compact('sms_id', 'phone', 'message', 'port'));
	...
	{
		return lib('sms')->get_input_receive($param);
	}
	...
	protected function _result($content)
	{
		...
		echo lib('sms')->make_feedback($content);
		exit();
	}
	...
	protected function _handler($handler, array $args = array())
	{
		return call_user_func_array(array(
			t('lib')->driver('sms_handler', $handler),
			'handle'
		), $args);
	}
	...
	{
		model('log_api')->log('sms_receive', array_filter(array(
			'get' 	=> t('input')->get(),
			'post' 	=> t('input')->post(),
		)), $message);
	}
