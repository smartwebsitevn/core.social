...
class Service extends MY_Controller
{
    ...
    function index()
    {
        ...
        $services=model('service')->filter_get_list(['show'=>1]);
    ...
    function _common_process()
    {
        ...
        //== Su ly danh muc
        $category = model('service_cat')->get_info($info->cat_id);
        ...
        $services=model('service')->filter_get_list(['show'=>1,'cat_id'=>$info->cat_id]);
        foreach($services as $row){
            $row =mod('service')->add_info_url($row);
        }
        $this->data['service_list'] =$services;
    ...
    protected function _breadcrumbs($category)
    {
        ...
        while ($parent->parent_id) {
            $parent = model('service_cat')->get_info($parent->parent_id);
