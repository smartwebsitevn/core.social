...
class Project_widget extends MY_Widget
{
    /**
     * Ham khoi dong
     */
    ...
    function filter($filters=array(),$temp = '', $temp_options = array())
    {
        ...
        // loc theo tag
        $where = array();
        $input['where']["table"] = 'project';
        //$input['where']["table_cat"] = 'project';
       ...
       // $tags = model('tag')->get_list(["status"=>1,"feature"=>1,'']);
        ...
        $temp = 'tpl::_widget/project/filter/filter_' . $temp;
    ...
    function same_cat($cat_id=null,$options=[],$temp = '', $temp_options = array())
    {
        ...
        $project_id = array_get($options, 'project_id', false);
    ...
    function feature($options=[], $temp = '', $temp_options = array())
    {
        ...
        // Get list
        $filter['feature'] = TRUE;
        //$this->data['url'] = site_url('project_list/home') . '?feature=1' . $type;
    ...
    function newest($options=[], $temp = '', $temp_options = array())
    {
        ...
        //$this->data['url'] = site_url('project_list/home') . '?order=id|desc' . $type;
    ...
    function viewest($options=[], $temp = '', $temp_options = array())
    {
        ...
        // Get list
        $input = array();
       // $this->data['url'] = site_url('project_list/home') . '?order=view_total|desc' . $type;
        $input['order'] = $order;
        $input['limit'] = array(0, $limit);
        $list = $this->get_list($filter, $input);
        //== Su ly hien thi temp hay tra ve du lieu
    ...
    {
        //Danh sach cach project_server dang duoc bat
        /* $inputs = array();
         $inputs['order'] = array('sort_order', 'asc');
         $inputs['where']['status'] = config('status_on', 'main');
         $project_servers = $this->project_server_model->get_list($inputs);*/
        ...
        foreach ($list as $row) {
            ...
            /*foreach (array('cat', 'tag') as $p) {
                $row->$p = model('project')->info_get($p, $row->id);
                foreach ($row->$p as $r) {
    ...
    function display_list($list, $temp = '', $temp_options = array())
    {
        ...
        $temp = 'tpl::_widget/project/display/list/list_' . $temp;
        // Su ly hien thi temp hay tra ve du lieu
        $return = array_get($temp_options, 'return_data', false);
        if ($return)
    ...
    function display_pagination($pages_config = array(), $temp = '', $temp_options = array())
    {
        ...
        $temp = (!$temp) ? 'default' : $temp;
        $temp = 'tpl::_widget/project/display/pagination/' . $temp;
        // Su ly hien thi temp hay tra ve du lieu
        $return = array_get($temp_options, 'return_data', false);
        if ($return)
            return $this->_display($this->_make_view($temp, __FUNCTION__), $return);
    ...
    function _ajax_pagination($filter, $limit)
    {
        ...
        $this->data['ajax_pagination_url'] = site_url('project_list/filter_ac?' . $pages_query . '&per_page=' . $limit);
        $total = model('project')->filter_get_total($filter);
