 ...
 */
	 ...
	 */
	protected function _get_list_set_input(array $input)
	{
		// Select
 ...
 */
	...
	protected function _join_get_sql_cur()
	{
		...
		if (count($this->db->qb_groupby) > 0)
		{
			...
			$sql .= implode(', ', $this->db->qb_groupby);
		}
 ...
 */
	 ...
	 * Lay tong so
	 */
	function filter_get_total(array $filter)
	{
 ...
 */
	...
	function handle_data_input(array $data)
	{
		...
		{
			// Params content
			if (in_array($p, $this->fields_type_content))
			{
				$v = handle_content($v, 'input');
			}
			
			// Params encode
			if (in_array($p, $this->fields_type_encode))
			{
				$v = security_encrypt($v, 'encode');
			}
			
			// Params list
			if (in_array($p, $this->fields_type_list))
			{
 ...
 */
	 ...
	 */
	protected function _translate_auto($row)
	{
		if (
