...
class VMS_Soap_Client {
  ...
  const DEBUG = FALSE;
 // const LOG_FILE_PATH =  '/home/thenapgr/public_html/application/libraries/paycard365/tmp/soap_vms.log';
 // const DATA_DIR = '/home/thenapgr/public_html/application/libraries/paycard365/tmp/store_vms.log';
  ...
  private static function getSession() {
    ...
    $DATA_DIR =  dirname(__FILE__) .self::DATA_DIR;
    $session_info = file_get_contents($DATA_DIR, TRUE);
    $session_info = explode("|", $session_info);
    if(!isset($session_info[1])) return false;
  ...
  function __construct($wsdl, $username, $password, $agent_id, $pin) {
    ...
    //$this->client     = new nusoap_client($this->wsdl, 'wsdl', FALSE, FALSE, FALSE, FALSE, 300, 300);
  ...
  private function doLogin() {
    ...
    $transid = (isset($result['return']['transid'])) ? $result['return']['transid'] : 0;
  ...
  public function doCardCharge($target, $pincard, $target_email, $target_mobile) {
    ...
    //neu mat phien session thi goi lai ham login
  ...
  public static function writeLog($result, $function, $ext_msg = '') {
    $transid = isset($result['return']['transid']) ? $result['return']['transid'] : 0;
    $msg = " [ {$transid} ] [ {$result['return']['status']} ] [ {$result['return']['message']} ] [ {$ext_msg} ]";

    $LOG_FILE_PATH =  dirname(__FILE__) .self::LOG_FILE_PATH;
    self::error_log($msg, $function, 'soap', 3, $LOG_FILE_PATH);
  }
  ...
  public static function error_log($message, $function, $type = "error", $level = 0, $destination = '') {
	...
	file_put_contents('paycard365_log.txt',$msg  .PHP_EOL,FILE_APPEND );
