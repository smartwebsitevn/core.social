<?php namespace Modules\Manufacture;
...
class Module extends \MY_Module
{
	...
	{
		t('lang')->load('admin/manuafacturer');
		$manuafacturers = array_pluck(model('manuafacturer')->get_list(), 'name', 'id');

		foreach (array('list', 'list_fixed') as $type) {
			foreach (array('', 1,2,3) as $k) {
				if (isset($config[$type]['setting']['manuafacturer_ids' . $k])) {
					$config[$type]['setting']['manuafacturer_ids'. $k]['values'] = $manuafacturers;
				}
				if (isset($config[$type]['setting']['layout_manuafacturer_ids' . $k])) {
					$config[$type]['setting']['layout_manuafacturer_ids'. $k]['values'] = $manuafacturers;
				}
	...
	{
		t("lang")->load("site/manuafacturer");
		$data = array();
	...
	{
		if (isset($widget->setting['manuafacturer_ids']) && $widget->setting['manuafacturer_ids']) {
			$filter = ['manuafacturer_ids' => $widget->setting['manuafacturer_ids']];
		...
		$list_feature=[];
		if(isset($widget->setting['layout_manuafacturer_ids']) && $widget->setting['layout_manuafacturer_ids'])
			$list_feature =$this->make_list(["ids"=>$widget->setting['layout_manuafacturer_ids'],'show'=>1]);
	...
	{
		$list1 =$this->make_list(["ids"=>$widget->setting['manuafacturer_ids1'],'show'=>1]);
		$list2 =$this->make_list(["ids"=>$widget->setting['manuafacturer_ids2'],'show'=>1]);
		$list3 =$this->make_list(["ids"=>$widget->setting['manuafacturer_ids3'],'show'=>1]);
		//$url_more= $this->make_url_more($widget,$filter);
		//pr($url_more,0);
	...
	protected function make_filter_get_list(array $args)
	{
		...
		foreach (array( 'manuafacturer_id', 'manuafacturer_ids','image') as $f) {
			if (isset($args[$f]) && !empty($args[$f])) {
				$filter[$f] = $args[$f];
			}
		}
	...
	{
		$list = model('manuafacturer')->filter_get_list($filter, $input);
		//pr_db($list);
	...
	/**
	 * Tao thong tin cho manuafacturer
	 *
	 * @param array
	 * @return array
	 */
	...
	{
		foreach ($list as $row) {
			$row = mod('manuafacturer')->add_info($row);
		}
		return $list;
	}
	protected function make_url_more($widget,$filter)
	{
		...
		if(isset($widget->setting['url_more']) && $widget->setting['url_more']){
			...
			$url_more = site_url('manuafacturer_list');
