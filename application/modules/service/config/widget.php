...
$_data_list =function($k='',$deny=array()){
	...
	if(!in_array('product_ids',$deny))
		$settings['product_ids'.$k]=[
			'type'   => 'select_multi',
			'name'   => 'service hiển thị',
			'value'  => '',
			'values' => [],
			'desc'   => 'Chọn service muốn hiển thị',
		];
	$settings['sp01'] =  array(
		'type' => 'separate',
	...
	if(!in_array('feature',$deny))
		$settings['feature'.$k]=[
			'type'   => 'select',
			'name'   => 'Chỉ hiển thị service được gán là hấp dẫn',
			'value'  => '',
			'values' => [
				'yes'     => 'Có',
				'no'    => 'Không',
	...
	if(!in_array('new',$deny))
        $settings['new'.$k]=[
            'type'   => 'select',
            'name'   => 'Chỉ hiển thị service được gán là mới',
            'value'  => '',
            'values' => [
                'yes'     => 'Có',
                'no'    => 'Không',
	...
	if(!in_array('soon',$deny))
		$settings['soon'.$k]=[
			'type'   => 'select',
			'name'   => 'Chỉ hiển thị service sắp ra mắt',
			'value'  => '',
			'values' => [
				'yes'     => 'Có',
				'no'    => 'Không',
	...
	if(!in_array('image',$deny))
		$settings['image'.$k]=[
			'type'   => 'select',
			'name'   => 'Chỉ hiển thị service có ảnh',
			'value'  => '',
			'values' => [
				'yes'     => 'Có',
				'no'    => 'Không',
...
$widget = [];
//================ List
$widget  ['list']['name'] = 'Danh sách service';
...
$widget  ['list']['setting']['style'] = [
	...
	'values' => [
		...
		'page_product' => 'Hiển thị ở trang service',*/
	],
	'values_opts' => array('value_required' => true),
];
//================ List fix
$widget  ['list_fixed']['name'] = 'Danh sách service tùy chọn';

for($i=1;$i<=3;$i++){
	$widget['list_fixed']["setting"]['sp'.$i]=['type' => 'separate',	'name' => 'Danh sách service '.$i];
	$widget['list_fixed']["setting"]['name'.$i]=['type' => 'text',	'name' => 'Tiêu đề '];
	$widget['list_fixed']['setting']['product_ids'.$i] =[
		'type'   => 'select_multi',
		'name'   => 'Danh sách service ',
		'value'  => '',
		'values' => [],
	];
}
