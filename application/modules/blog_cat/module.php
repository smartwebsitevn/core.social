<?php namespace Modules\Blog_cat;
...
class Module extends \MY_Module
{
    ...
    {
        t('lang')->load('admin/blog_cat');
        $cat_ids = array_pluck(model('blog_cat')->get_hierarchy_data(), '_name', 'id');

        foreach (array('list',) as $type) {
            foreach (array('', 1,2,3) as $k) {
                if (isset($config[$type]['setting']['id' . $k])) {
                    $config[$type]['setting']['id'. $k]['values'] = $cat_ids;
                }
    ...
    protected function make_filter_get_list(array $args)
    {
        ...
        foreach (array(  'id','image','banner','icon') as $f) {
            if (isset($args[$f]) && !empty($args[$f])) {
                $filter[$f] = $args[$f];
            }
        }
    ...
    protected function make_info(array $list)
    {
        foreach ($list as $row) {
            ...
            $total_blog= model('blog')->filter_get_total(['cat_id'=>$row->id]);
            $row->_total_blog =number_format($total_blog);
        }
       // pr($list);
        return $list;
    }
