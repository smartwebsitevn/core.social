<?php namespace Modules\Product_cat;
...
class Module extends \MY_Module
{
    ...
    {
        t('lang')->load('admin/product_cat');
        $cat_ids = array_pluck(model('product_cat')->get_hierarchy_data(), '_name', 'id');
    ...
    {
        $list = model('product_cat')->filter_get_list($filter, $input);
        //pr_db($list);
    ...
    protected function make_url_more($widget,$filter)
    {
        ...
        if(isset($widget->setting['url_more']) && $widget->setting['url_more']){
            ...
            $url_more = site_url('product_list');
