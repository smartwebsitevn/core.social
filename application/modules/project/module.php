...
class Module extends \MY_Module
{
	...
	{
		t('lang')->load('admin/project');
		$projects = array_pluck(model('project')->get_list(), 'name', 'id');

		foreach (array('list', 'list_fixed') as $type) {
			foreach (array('', 1,2,3) as $k) {
				if (isset($config[$type]['setting']['project_ids' . $k])) {
					$config[$type]['setting']['project_ids'. $k]['values'] = $projects;
				}
				if (isset($config[$type]['setting']['layout_project_ids' . $k])) {
					$config[$type]['setting']['layout_project_ids'. $k]['values'] = $projects;
				}
	...
	{
		if (isset($widget->setting['project_ids']) && $widget->setting['project_ids']) {
			$filter = ['project_ids' => $widget->setting['project_ids']];
		...
		$list_feature=[];
		if(isset($widget->setting['layout_project_ids']) && $widget->setting['layout_project_ids'])
			$list_feature =$this->make_list(["ids"=>$widget->setting['layout_project_ids'],'show'=>1]);
	...
	{
		$list1 =$this->make_list(["ids"=>$widget->setting['project_ids1'],'show'=>1]);
		$list2 =$this->make_list(["ids"=>$widget->setting['project_ids2'],'show'=>1]);
		$list3 =$this->make_list(["ids"=>$widget->setting['project_ids3'],'show'=>1]);
		//$url_more= $this->make_url_more($widget,$filter);
		//pr($url_more,0);
	...
	protected function make_filter_get_list(array $args)
	{
		...
		foreach (array( 'project_id', 'project_ids','image') as $f) {
			if (isset($args[$f]) && !empty($args[$f])) {
				$filter[$f] = $args[$f];
			}
		}
	...
	protected function make_url_more($widget,$filter)
	{
		...
		if(isset($widget->setting['url_more']) && $widget->setting['url_more']){
			...
			$url_more = site_url('project_list');
