...
class Module extends \MY_Module
{
	...
	{
		t('lang')->load('admin/aboutus');
		$ids = array_pluck(model('aboutus')->get_list(), 'name', 'id');
		$cat_ids = array_pluck(model('aboutus_cat')->get_hierarchy_data(), '_name', 'id');
	...
	protected function make_url_more($widget,$filter)
	{
		...
		if(isset($widget->setting['url_more']) && $widget->setting['url_more']){
			...
			$url_more = site_url('aboutus_list');
