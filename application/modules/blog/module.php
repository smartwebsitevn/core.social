...
class Module extends \MY_Module
{
	...
	{
		t('lang')->load('admin/blog');
		$blogs = array_pluck(model('blog')->get_list(), 'name', 'id');

		foreach (array('list', 'list_fixed') as $type) {
			foreach (array('', 1,2,3) as $k) {
				if (isset($config[$type]['setting']['blog_ids' . $k])) {
					$config[$type]['setting']['blog_ids'. $k]['values'] = $blogs;
				}
				if (isset($config[$type]['setting']['layout_blog_ids' . $k])) {
					$config[$type]['setting']['layout_blog_ids'. $k]['values'] = $blogs;
				}
	...
	{
		if (isset($widget->setting['blog_ids']) && $widget->setting['blog_ids']) {
			$filter = ['blog_ids' => $widget->setting['blog_ids']];
		...
		$list_feature=[];
		if(isset($widget->setting['layout_blog_ids']) && $widget->setting['layout_blog_ids'])
			$list_feature =$this->make_list(["ids"=>$widget->setting['layout_blog_ids'],'show'=>1]);
	...
	{
		$list1 =$this->make_list(["ids"=>$widget->setting['blog_ids1'],'show'=>1]);
		$list2 =$this->make_list(["ids"=>$widget->setting['blog_ids2'],'show'=>1]);
		$list3 =$this->make_list(["ids"=>$widget->setting['blog_ids3'],'show'=>1]);
		//$url_more= $this->make_url_more($widget,$filter);
		//pr($url_more,0);
	...
	protected function make_filter_get_list(array $args)
	{
		...
		foreach (array( 'blog_id', 'blog_ids','image') as $f) {
			if (isset($args[$f]) && !empty($args[$f])) {
				$filter[$f] = $args[$f];
			}
		}
