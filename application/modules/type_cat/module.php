<?php namespace Modules\Type_cat;
...
class Module extends \MY_Module
{
    ...
    {
        t('lang')->load('admin/type_cat');
        $cat_ids = array_pluck(model('type_cat')->get_hierarchy_data(), '_name', 'id');
    ...
    {
        $list = model('type_cat')->filter_get_list($filter, $input);
        //pr_db($list);
