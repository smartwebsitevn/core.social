...
$_data_list =function($k='',$deny=array()){
	...
	if(!in_array('feature',$deny))
		$settings['feature'.$k]=[
			'type'   => 'select',
			'name'   => 'Chỉ hiển thị blog_author được gán là hấp dẫn',
			'value'  => '',
			'values' => [
				'yes'     => 'Có',
				'no'    => 'Không',
	...
	if(!in_array('new',$deny))
        $settings['new'.$k]=[
            'type'   => 'select',
            'name'   => 'Chỉ hiển thị blog_author được gán là mới',
            'value'  => '',
            'values' => [
                'yes'     => 'Có',
                'no'    => 'Không',
	...
	if(!in_array('soon',$deny))
		$settings['soon'.$k]=[
			'type'   => 'select',
			'name'   => 'Chỉ hiển thị blog_author sắp ra mắt',
			'value'  => '',
			'values' => [
				'yes'     => 'Có',
				'no'    => 'Không',
	...
	if(!in_array('image',$deny))
		$settings['image'.$k]=[
			'type'   => 'select',
			'name'   => 'Chỉ hiển thị blog_author có ảnh',
			'value'  => '',
			'values' => [
				'yes'     => 'Có',
				'no'    => 'Không',
...
$widget = [];
//================ List
$widget  ['list']['name'] = 'Danh sách blog_author';
...
$widget  ['list']['setting']['style'] = [
	...
	'values' => [
		...
		'page_product' => 'Hiển thị ở trang blog_author',*/
	],
	'values_opts' => array('value_required' => true),
];
//================ List fix
$widget  ['list_fixed']['name'] = 'Danh sách blog_author tùy chọn';

for($i=1;$i<=3;$i++){
	$widget['list_fixed']["setting"]['sp'.$i]=['type' => 'separate',	'name' => 'Danh sách blog_author '.$i];
	$widget['list_fixed']["setting"]['name'.$i]=['type' => 'text',	'name' => 'Tiêu đề '];
	$widget['list_fixed']['setting']['id'.$i] =[
		'type'   => 'select_multi',
		'name'   => 'Danh sách blog_author ',
		'value'  => '',
		'values' => [],
	];
}
