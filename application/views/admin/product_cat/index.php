...
/*$_macro['toolbar_sub'] = array(
    array('url' => admin_url('news'), 'title' => lang('mod_news') ),
    array('url' => admin_url('news_cat'), 'title' => lang('mod_news_cat'),'attr'=>array('class'=>'active') )
);*/
...
$_data_parent = function ($row) {
    ob_start(); ?>
    <?php
    if($row->parent_id && isset($row->_name) && $row->_name)
         echo $row->_name . '<br>';
    ?>
    <?php return ob_get_clean();
};
