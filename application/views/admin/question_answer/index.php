...
$_row_action = function ($row) {
    ...
    ?>
    <a href="<?php echo $row->_url_view ?>" title="<?php echo lang('view'); ?>" data-width="60%" data-height="80%"
       class="btn btn-primary btn-xs lightbox  "><?php echo lang('view'); ?></a>
    <?php //endif;
    ?>
    ...
    <?php if ($row->_can_del): ?>
        <a _url="<?php echo $row->_url_del; ?>" notice="<?php echo lang('notice_confirm_del'); ?>"
           title="<?php echo lang('delete'); ?>" class="btn btn-danger btn-xs  verify_action "
    ...
    <?php if ($row->status): ?>
        <a href="#0" class="btn btn-warning btn-xs  verify_action mt5"
           ...
           _url="<?php echo admin_url('question_answer/unverify/' . $row->id) ?>">Hủy xác thực</a>
    <?php else: ?>
        <a href="#0" class="btn btn-primary btn-xs  verify_action mt5"
           notice="Bạn có chắc muốn xác thực bình luận này?<?php ?>"
           _url="<?php echo admin_url('question_answer/verify/' . $row->id) ?>">Xác thực</a>
    <?php endif; ?>
...
foreach ($list as $row) {
    ...
    $r['question'] = character_limiter(strip_tags($row->question), 20);
    $r['answer'] = character_limiter(strip_tags($row->answer), 20);
    if ($row->user)
