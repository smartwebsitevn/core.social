	...
	<form class="form" id="form" action="<?php echo $action; ?>" method="post">
		<fieldset>
			<div class="widget">
						...
						<div class="formRight">
							<select name="group_id" _autocheck="true" class="left" style="width:257px;">
								...
								<?php foreach ($groups as $group): ?>
									<option value="<?php echo $group->id; ?>" <?php if($group->id == $info->group_id){echo 'selected';}?>>
										<?php echo $group->name; ?>
									</option>
								<?php endforeach; ?>
							</select>
				...
				<!-- 
				<div class="formRow">
					<label class="formLeft"><?php echo lang('yahoo_image'); ?></label>
					<div class="formRight">
					    <?php for ($i = 1;$i<=14;$i++):?>
						    <div style="float:left;margin:10px;width:100px;height:50px">
		                 		<input type="radio" name="yahoo_image" value="<?php echo $i?>" id="param_yahoo_image" />
		                 		<img style='max-width:70px' src='http://opi.yahoo.com/online?u=tuyenht90&m=g&t=<?php echo $i?>' />
	                 		</div>
					    <?php endfor;?>
					</div>
					<div class="clear"></div>
