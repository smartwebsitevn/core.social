...
$_macro['toolbar'] = array(
    array(
        'url' 	=> admin_url('log_activity/admin'),
        'title' =>lang('title_log_activity_admin'),
        'attr'=>array('class'=>'btn btn-primary'),
        'icon' => 'user',
    ),
    array(
        'url' 	=> admin_url('log_activity/user'),
        'title' => lang('title_log_activity_user'),
        'attr'=>array('class'=>'btn btn-primary'),
        'icon' => 'group',
    ),
);
...
foreach ($list as $row) {
   // pr($row);
    ...
    if (isset($row->user) && $row->user)
        $user = t('html')->a(admin_url('user') . "?user_id={$row->user}", $_acc, ['target' => 'target']);
