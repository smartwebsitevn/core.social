...
$_macro['table']['filters'] = array(
    ...
    array(
        ...
        'param' => 'author_id',
        'type' => 'select',
        'value' => isset($filter['author_id']) && $filter['author_id'] != '' ? $filter['author_id'] : '-1',
        'values_row' => array($authors, 'id', 'name')
    ),
     /*array(
        'type' => 'select',
        'param' => 'price_option',
...
$_data_author = function ($row) {
    ob_start(); ?>
    <?php if (isset($row->_author_name) && $row->_author_name) echo $row->_author_name . '<br>'; ?>
    <?php return ob_get_clean();
};
