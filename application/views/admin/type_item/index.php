...
$_macro['table']['filters'] = array(
    ...
    array(
        'name' => lang('type'),
         'param' => 'type_id',
        'type' => 'select',
        'value' => isset($filter['type_id']) && $filter['type_id'] != '' ? $filter['type_id'] : '-1',
        'values_row' => array($types, 'id', 'name')
    ),
    array(
        'name' => lang('feature'),
        'type' => 'select',
...
$_data_name = function ($row) {
    ...
    <br>
    <i class="fa fa-eye" title="Số lượt xem"></i> <?php echo $row->view_total ?>
...
$_data_type = function ($row) {
    ob_start(); ?>
    <?php if (isset($row->_type_name) && $row->_type_name) echo $row->_type_name . '<br>'; ?>
    <?php return ob_get_clean();
};
