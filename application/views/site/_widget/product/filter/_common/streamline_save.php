<?php
$stream = model('product')->get_streamline_save($user->id);
if ($stream->min):
        ...
        <ul class="list-group  timeline">
            <?php if ($min->year < $max->year): ?>
                ...
                <?php foreach (range($max->year-1, $min->year, -1) as $y): ?>
                    ...
                    ?>
                    <li class="list-group-item act-filter " data-name="created" data-value="<?php echo $filter['created'].'|'.$filter['created_to'] ?>"><a href="#0"><b>Năm <?php echo $y  ?></b> : <?php echo number_format($total) ?> bài đã lưu </a></li>
