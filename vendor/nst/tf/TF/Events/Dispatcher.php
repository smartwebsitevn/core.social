...
class Dispatcher {
	 ...
	 *
	 * @param  \TF\Container\Container  $container
	 * @return void
	 */
	...
	public function makeListener($listener)
	{
		if (is_string($listener))
		{
			...
			$method		= (isset($listener[1])) ? $listener[1] : '';
			$listener 	= $listener[0];
			
			if ( ! is_callable($listener))
			{
				$listener = $this->createClassListener($listener, $method);
			}
		}
	 ...
	 * @return \Closure
	 */
	public function createClassListener($listener, $method = '')
	{
		$container = $this->container;

		return function() use ($listener, $method, $container)
		{
			...
			$method = count($segments) == 2 ? $segments[1] : $method;
